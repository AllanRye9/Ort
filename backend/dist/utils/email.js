...
exports.sendImageApprovedEmail = sendImageApprovedEmail;
exports.sendImageRejectedEmail = sendImageRejectedEmail;
exports.sendListingApprovedEmail = sendListingApprovedEmail;
exports.sendListingExpiredEmail = sendListingExpiredEmail;
exports.sendListingLikedEmail = sendListingLikedEmail;
...
function createTransport() {
    ...
    if (!host || !user || !pass) {
        logger_1.logger.warn('SMTP not configured – emails will be logged only');
        return null;
    }
...
async function send(to, subject, html) {
    if (resend) {
        try {
            ...
            logger_1.logger.info(`Email sent to ${to} via Resend: ${subject}`);
            return;
        }
        catch (err) {
            logger_1.logger.error(`Resend delivery failed for ${to}: ${String(err)}`);
        }
    }
    ...
    try {
        ...
        logger_1.logger.error(`Failed to send email to ${to}: ${String(err)}`);
        throw err;
    }
}
// ─────────────────────────────────────────────────────────────────────────
...
async function sendEmailVerificationEmail(to, name, verificationToken) {
...
async function sendImageApprovedEmail(to, name, listingTitle) {
...
async function sendImageRejectedEmail(to, name, reason, listingTitle) {
...
async function sendListingApprovedEmail(to, name, listingTitle, expiresAt) {
...
async function sendListingExpiredEmail(to, name, listingTitle) {
...
async function sendListingLikedEmail(to, ownerName, listingTitle, listingId, likerName, likerUserId) {
...
async function sendSubscriptionActivatedEmail(to, name, packageName, expiresAt) {
