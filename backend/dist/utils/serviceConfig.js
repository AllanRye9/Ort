...
exports.validateAndLogServiceConfig = validateAndLogServiceConfig;
exports.getServiceReadiness = getServiceReadiness;
...
function warnPartialConfig(serviceName, keys) {
    const missingKeys = missing(keys);
    if (missingKeys.length === 0)
        return;
    if (!hasAny(keys))
        return;
    logger_1.logger.warn(`${serviceName} partially configured. Missing: ${missingKeys.join(', ')}. ` +
        'Service will not be used until all required variables are set.');
}
function validateAndLogServiceConfig() {
    ...
    logger_1.logger.info(`Service config: JWT=ready, ` +
    ...
    const isEphemeralHost = Boolean(process.env.RAILWAY_ENVIRONMENT_ID || process.env.RAILWAY_PROJECT_ID);
    if (!s.s3 && isEphemeralHost) {
        logger_1.logger.warn('⚠ S3 storage is NOT configured (ACCESS_KEY_ID / SECRET_ACCESS_KEY / BUCKET missing) ' +
