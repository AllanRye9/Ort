...
function parseDocumentText(text) {
    ...
    for (const line of lines) {
        ...
        if (/^(title|name|product|item)$/.test(key)) {
            ...
            if (cur)
                result.currency = cur[1].toUpperCase();
        }
            ...
            if (/uae|emirates|dubai|abu.?dhabi/.test(norm))
                result.country = 'UAE';
            else if (/uganda|kampala/.test(norm))
                result.country = 'UGANDA';
            else if (/kenya|nairobi/.test(norm))
                result.country = 'KENYA';
            else if (/china|beijing|shanghai/.test(norm))
                result.country = 'CHINA';
        }
        else if (/^(currency)$/.test(key)) {
            const valid = CURRENCY_PATTERN.split('|');
            if (valid.includes(val.toUpperCase()))
                result.currency = val.toUpperCase();
        }
        else {
            unmatched.push(line);
        }
    ...
    if (result.price === null) {
        const m = text.match(new RegExp(`(?:${CURRENCY_PATTERN})?\\s*([\\d,]+\\.?\\d*)|([\\d,]+\\.?\\d*)\\s*(?:${CURRENCY_PATTERN})`, 'i'));
        if (m) {
            const raw = m[1] ?? m[2];
 ...
 */
router.post('/convert', auth_1.authenticate, 
...
(req, res, next) => {
    ...
    try {
        ...
        if (multerReq.file) {
            ...
            return next((0, errorHandler_1.createError)('Provide a document file (multipart field "document") or a JSON body with a "text" field', 400));
        }
        if (text.length > 50000) {
            return next((0, errorHandler_1.createError)('Document too large – maximum 50,000 characters', 400));
        }
